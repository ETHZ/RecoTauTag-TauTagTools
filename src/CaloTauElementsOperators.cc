...
vector<pair<math::XYZPoint,float> > 
CaloTauElementsOperators::EcalRecHitsInCone(const math::XYZVector& coneAxis,const string coneMetric,const double coneSize,const double EcalRecHit_minEt,const vector<pair<math::XYZPoint,float> >& myEcalRecHits)const{
...
vector<pair<math::XYZPoint,float> > 
CaloTauElementsOperators::EcalRecHitsInAnnulus(const math::XYZVector& coneAxis,const string innerconeMetric,const double innerconeSize,const string outerconeMetric,const double outerconeSize,const double EcalRecHit_minEt,const vector<pair<math::XYZPoint,float> >& myEcalRecHits) const {     
   vector<pair<math::XYZPoint,float> > theFilteredEcalRecHits;
   for (vector<pair<math::XYZPoint,float> >::const_iterator iEcalRecHit=myEcalRecHits.begin();iEcalRecHit!=myEcalRecHits.end();++iEcalRecHit) {
      if ((*iEcalRecHit).second*fabs(sin((*iEcalRecHit).first.theta()))>EcalRecHit_minEt)theFilteredEcalRecHits.push_back(*iEcalRecHit);
   }  
...
vector<pair<math::XYZPoint,float> >
CaloTauElementsOperators::EcalRecHitsInCone(const math::XYZVector& coneAxis,const string coneMetric,const double coneSize,const double EcalRecHit_minEt) const 
{
...
vector<pair<math::XYZPoint,float> >
CaloTauElementsOperators::EcalRecHitsInAnnulus(const math::XYZVector& coneAxis,const string innerconeMetric, const double innerconeSize,
                                               const string outerconeMetric, const double outerconeSize, const double EcalRecHit_minEt) const 
{
   //this function exists only to provide compatability w/ CMSSW_2_2_3 out of the box.  Newer versions recompute the interesting rechits inside RecoTau
